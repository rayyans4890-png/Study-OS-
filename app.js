...
function renderHome() {
  ...
  if (todays.length) {
    ...
    todayHTML =
      '<p class="empty-msg">Nothing due today.</p>';
  }
  ...
  if (ne) {
    ...
    examHTML =
      '<p class="empty-msg">No exams scheduled.</p>';
  }
  ...
  if (secs > 0) {
    ...
    focusHTML =
      '<p class="empty-msg">No study time yet today.</p>';
  }

  /*
    Home page is intentionally simple.
    The old Quick Look sidebar was removed.
  */
...
function renderPlanner() {
  ...
  if (dayTasks.length) {
    ...
    dayHTML =
      '<p class="empty-msg">Nothing scheduled this day.</p>';
  }
  ...
  var selectedDate = parseDate(selectedDay);

  var weekday = selectedDate.toLocaleDateString("en-US", {
    weekday: "long"
  });
    ...
    '<div class="planner-content">' +
      ...
      '<div class="cal">' +
        ...
        '<div class="cal-head">' +
          ...
          '<button class="btn ghost small" onclick="shiftMonth(-1)">' +
            "←" +
          "</button>" +
...
function renderTasks() {
  ...
  el.innerHTML =
    ...
    '<form class="add-form" onsubmit="submitTaskForm(event)">' +
      ...
      '<p class="edit-hint hidden" id="taskEditHint">' +
        'Editing a task — ' +
...
function renderExams() {
  ...
  if (sorted.length) {
    ...
    rowsHTML =
      '<p class="empty-msg">' +
      "No exams yet. Add one when a test comes up." +
      "</p>";
  }
  ...
  el.innerHTML =
    ...
    '<form class="add-form" onsubmit="submitExamForm(event)">' +
      ...
      '<p class="edit-hint hidden" id="examEditHint">' +
        'Editing an exam — ' +
        '<button type="button" class="text-btn" onclick="cancelExamEdit()">cancel</button>' +
      "</p>" +
  ...
  if (subjects.length) {
    ...
    for (var i = 0; i < subjects.length; i++) {
      ...
      for (var j = 0; j < sessions.length; j++) {
        if (
          sessions[j].subject &&
          sessions[j].subject.toLowerCase() ==
          s.name.toLowerCase()
        ) {
    ...
    cardsHTML =
      '<p class="empty-msg">' +
      "No subjects yet. Add your classes here." +
      "</p>";
  }
...
function renderFocus() {
  ...
  if (
    ...
    timerHTML =
      ...
      '<div class="timer-progress">' +
        '<div class="fill" id="timerFill" style="width:' +
        pct +
        '%"></div>' +
      "</div>" +
    ...
    timerHTML =
      ...
      '<p class="muted">' +
        "Pick a length and a subject, then start." +
      "</p>" +
  ...
  el.innerHTML =
    '<div class="page-head">' +
      ...
      '<p>' +
        "Set a timer and study one thing at a time." +
      "</p>" +
